...
onBeforeMount(() => {
  ...
  plan.value = planService.createPlan("", planJson, "")
    ...
    <pane ref="planEl" class="plan grab-bing position-relative">
      <svg width="100%" height="100%">
        <g :transform="transform">
          <!-- Links -->
          <path v-for="(link, index) in toCteLinks" :key="`linkcte${index}`" :d="lineGen(link)" stroke="#B3D7D7"
            :stroke-width="edgeWeight(
              link.target.data[NodeProp.ACTUAL_ROWS_REVISED]
            )
              " fill="none" />
          <path v-for="(link, index) in layoutRootNode?.links()" :key="`link${index}`" :d="lineGen(link)" :class="{
            'never-executed': isNeverExecuted(link.target.data),
          }" stroke="grey" :stroke-width="edgeWeight(
  link.target.data[NodeProp.ACTUAL_ROWS_REVISED]
)
  " stroke-linecap="square" fill="none" />
          <foreignObject v-for="(item, index) in layoutRootNode?.descendants()" :key="index" :x="item.x - item.xSize / 2"
            :y="item.y" :width="item.xSize" height="1" ref="root">
            <plan-node :node="item.data" class="d-flex justify-content-center position-fixed" />
          </foreignObject>
          <g v-for="cte in ctes" :key="cte.data.nodeId">
            <rect :x="getLayoutExtent(cte)[0] - padding / 4" :y="getLayoutExtent(cte)[2] - padding / 2" :width="getLayoutExtent(cte)[1] -
    ...
    " stroke="#cfcfcf" stroke-width="2" fill="#cfcfcf" fill-opacity="10%" rx="5" ry="5"></rect>
            <path v-for="(link, index) in cte.links()" :key="`link${index}`" :d="lineGen(link)" stroke="grey"
              :stroke-width="edgeWeight(
                link.target.data[NodeProp.ACTUAL_ROWS_REVISED]
              )
                " stroke-linecap="square" fill="none" />
            <foreignObject v-for="(item, index) in cte.descendants()" :key="index" :x="item.x - item.xSize / 2"
              :y="item.y" :width="item.xSize" height="1" ref="root">
              <plan-node :node="item.data" class="d-flex justify-content-center position-fixed" />
            </foreignObject>
          </g>
